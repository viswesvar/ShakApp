...
public class ShakeDetector implements SensorEventListener {
    public static enum Tilt {FLAT, LEFT, RIGHT}
    ...
    private static int currShakes = 0;
    ...
    @Override public void onSensorChanged(SensorEvent event) {
        ...
        listener.onAxisChanged(event.values[1]);
        ...
        if(!accelerating){
            Tilt tilt = isTilting(event);
            if(!tilt.equals(Tilt.FLAT))
    ...
    public static Tilt isTilting(SensorEvent event){
        int inclination = (int) Math.round(Math.toDegrees(Math.acos(event.values[2])));
        int rotation = (int) Math.round(Math.toDegrees(Math.atan2(event.values[0], event.values[1])));
    ...
    private boolean isAccelerating(SensorEvent event) {
        ...
        final double magnitudeSquared = ay * ay; //ax * ax + ay * ay + az * az;
